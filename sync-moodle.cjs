...
async function syncAssignments() {
  ...
  for (const a of rows) {
    ...
    if (sync.autoGenerateRubric) {
      ...
      // Try to load criteria from teacherbot DB ai_config
      let criteria = [];
      try {
        const raw = tdb.prepare("SELECT value FROM ai_config WHERE key = 'rubric_auto_criteria'").get();
        if (raw && raw.value) {
          criteria = JSON.parse(raw.value);
        }
      } catch(e) {
        console.log('   ⚠️  No se pudo leer rubric_auto_criteria de ai_config, usando defaults');
      }

      // Fallback to hardcoded defaults if DB has no config
      if (!criteria || criteria.length === 0) {
        criteria = [
          { criterion: 'Contenido y Precision', weight_pct: 40, description: 'Calidad, correccion y profundidad del contenido presentado.' },
          { criterion: 'Estructura y Organizacion', weight_pct: 25, description: 'Claridad en la estructura, orden logico y formato.' },
          { criterion: 'Creatividad y Originalidad', weight_pct: 20, description: 'Aporte personal, ideas originales y creatividad.' },
          { criterion: 'Presentacion y Ortografia', weight_pct: 15, description: 'Limpieza, ortografia, gramatica y referencias.' },
        ];
      }
      ...
      for (const cr of criteria) {
        const weight = cr.weight_pct || (100 / criteria.length);
        const points = Math.round(totalPoints * (weight / 100) * 10) / 10;
        insertRubric.run(tbId, cr.criterion, points, cr.description || '', sortOrder);
        sortOrder++;
      }
    }
  }
